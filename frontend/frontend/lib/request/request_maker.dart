...
class RequestMaker {
  ...
  static Future<ResultRequest> calculate({required String expression}) async {
    ...
    return http
      ...
      .then((response) {
        ...
        return FailedRequest(errorMsg: 'Request failed with status: ${response.statusCode}.');
      })
      .catchError((onError) => FailedRequest(errorMsg: 'Exception in RequestMaker: $onError'));
  }
  ...
  static Future<bool> pushHistory({required HistoryCalculation value}) async {
    ...
    return http
      .post(Uri.parse(url), body: jsonEncode(value))
      .then((response) {
        if (response.statusCode == 200) {
          return true;
        }
