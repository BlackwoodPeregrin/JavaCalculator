...
class _HomePageState extends State<HomePage>
  ...
  Future _onPressed(
      {required BuildContext context, required ButtonId id}) async {
    ...
    if (id.isEqual()) {
      ...
      final response = await RequestMaker.calculate(
          expression: _replaceSpecSymbolCalcString());
      ...
      if (response is SuccessRequestCalc) {
        ...
        if (await RequestMaker.pushHistory(value: historyValue) == true) {
          ...
          await _showMessageBox(context,
              title: _titleMsgBox, msg: 'Add value to history failed');
        }
      ...
      if (response is FailedRequest) {
        // ignore: use_build_context_synchronously
        await _showMessageBox(context,
            title: _titleMsgBox, msg: response.errorMsg);
        return;
      }
      ...
      await _showMessageBox(context,
          title: _titleMsgBox,
          msg: 'Not Expected type response request: ${response.runtimeType}');
      return;
    }
  ...
  Future<List<HistoryCalculation>?> _loadCalculationHistory() async {
    ...
    if (response is SuccessRequestHistoryCalc) {
      ...
      await _showMessageBox(context,
          title: _titleMsgBox,
          msg: 'Load calculation history failded: ${response.errorMsg}');
    } else {
      // ignore: use_build_context_synchronously
      await _showMessageBox(context,
          title: _titleMsgBox,
          msg:
              'Load calculation history failded: Not Expected type response request: ${response.runtimeType}');
    }
    return null;
  }

  Future _showMessageBox(BuildContext context,
  ...
  Future _openModal(BuildContext contex) => showModalBottomSheet(
        context: context,
        builder: (BuildContext context) {
          return Container(
            ...
            child: SingleChildScrollView(
              child: Column(
                children: <Widget>[
                  ...
                  const SizedBox(
                      height: 50), // Установите нужный вам отступ между Row
                  Row(
                    ...
                    children: [
                      ElevatedButton(
                        onPressed: () async {
                          ...
                          if (await RequestMaker.clearHistory()) {
                            historyCalc = [];
                          } else {
                            // ignore: use_build_context_synchronously
                            await _showMessageBox(context,
                                title: _titleMsgBox,
                                msg: 'Clear history calculation failed.');
                          }
                        },
  ...
  String _replaceSpecSymbolCalcString() => calcString
      .replaceAll(ButtonNameConverter.convert(buttonId: ButtonId.sqrt), 'sqrt')
      .replaceAll(
          ButtonNameConverter.convert(buttonId: ButtonId.pi), pi.toString());
}
